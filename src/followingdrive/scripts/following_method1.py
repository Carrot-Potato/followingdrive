...
class Detect_Cal:
  def __init__(self):
    self.angle_pub = rospy.Publisher("angle_distance", angle_distance)
...
#        print('point : ', box_distance[i])
    ...
    if min_distance != 40:
        ...
        cx = self.mtx[0][2]
        cy = self.mtx[1][2]
        RR = cv2.Rodrigues(self.rvec)
        R = RR[0]
        #invR = np.linalg.inv(R[0])
        T = self.tvec
        RT = [[R[0][0], R[0][1], R[0][2], T[0][0]],
              [R[1][0], R[1][1], R[1][2], T[1][0]],
              [R[2][0], R[2][1], R[2][2], T[2][0]],
              [0, 0, 0, 1]]
        matrix = np.linalg.inv(RT)
        
        x_3d = round((min_distance/fx)*(x_2d-cx),3)
        y_3d = round((min_distance/fy)*(y_2d-cy),3)
        th = math.pi-(math.acos(x_3d/min_distance))
        z_3d = round(math.sin(th) * min_distance,3)
        x_3D = round(matrix[0][2] * z_3d + matrix[0][0] * x_3d + matrix[0][1] * y_3d + matrix[0][3] * 1,3)
        y_3D = round(matrix[1][2] * z_3d + matrix[1][0] * x_3d + matrix[1][1] * y_3d + matrix[1][3] * 1,3)
        z_3D = round(matrix[2][2] * z_3d + matrix[2][0] * x_3d + matrix[2][1] * y_3d + matrix[2][3] * 1,3)
        distance = min_distance
        angle = th
        print('OK')
    
    self.ad_msg.angle = angle
    self.ad_msg.distance = min_distance
    self.ad_msg.header.frame_id = 'lidar_frame'
    self.ad_msg.header.stamp = rospy.Time.now()
    cv2.waitKey(3)
    
    try:
      self.angle_pub.publish(self.ad_msg)
    except CvBridgeError as e:
      print(e)
        
def main(args):
  print(cv2.__version__)
  rospy.init_node('DetectCal', anonymous=True)  
  DC = Detect_Cal()
  try:
    rospy.spin()
